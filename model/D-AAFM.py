...
class Config(object):
    ...
    def __init__(self, dataset, data_type, lr, batch_size, test_batch_size, ):
        ...
        self.save_path = os.path.join(data_dir, 'save_path/'+data_type+'_E_AAFM.bin')
...
class Model(BaseModel):
    def __init__(self, num_one_hot, emb_size=64, ):
        ...
        self.lam_scaler = MinMaxScaler(feature_range=(1, 100))
        self.hum_scale_0 = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.hum_scale_1 = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.hum_scale_2 = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.hum_scale_3 = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.hum_scale_4 = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)
    ...
    def forward(self, X_sparse_one, label, freq, data_type, loss_fct, epoch, train, humidity):
        ...
        if train:
            ...
            if epoch > 0:
                ...
                self.lam_scaler.fit(-freq.cpu().detach())
                scaled_lam = torch.from_numpy(self.lam_scaler.transform(-freq.cpu().detach()).astype(np.float32))
                if torch.cuda.is_available():
                    adv_loss_fct = nn.BCELoss(weight=scaled_lam.squeeze().cuda(), size_average=None, reduce=None, reduction='mean')
                else:
                    adv_loss_fct = nn.BCELoss(weight=scaled_lam.squeeze(), size_average=None, reduce=None, reduction='mean')
                ...
                eplison = {}
                humidity = nn.functional.normalize(1/humidity, p=2, dim=1)                    
                w_lst = [self.hum_scale_0, self.hum_scale_1, self.hum_scale_2, self.hum_scale_3, self.hum_scale_4]
                reg_eps = 0
                for fea_id in perturbed_fea_id:
                    eps = self.softplus((humidity[:, fea_id])*w_lst[fea_id])
                    reg = torch.norm(eps, p=2, dim=0)
                    reg_eps += reg
                    eplison[fea_id] = eps.squeeze()
                ...
                for fea_id in perturbed_fea_id:
                    for i in range(one_hot_embedding[fea_id].shape[0]):
                        ...
                        eps_delta_i = torch.clamp(delta_i*eplison[fea_id][i], -eplison[fea_id][i], eplison[fea_id][i])
                ...
                loss = loss_adv +  + math.log10(reg_eps.sum())/epoch
